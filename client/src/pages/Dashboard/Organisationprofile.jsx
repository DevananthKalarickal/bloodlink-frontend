...
import { FaUserTie, FaPhone, FaEnvelope, FaGlobe, FaMapMarkerAlt, FaBuilding } from "react-icons/fa";
...
const Donation = () => {
  ...
  const handleSubmit = async () => {
      ...
      try {
        console.log("Editing Record ID:", editingRecord._id); // Debugging ID
        console.log("Edit Form Data:", editForm); // Debugging form data
    
        const response = await API.put(`/inventory/update-inventory/${editingRecord._id}`, {
          ...editForm,
        });
        ...
        if (response.data.success) {
          setData((prevData) =>
            prevData.map((record) =>
              record._id === editingRecord._id ? { ...record, ...editForm } : record
            )
          );
          setEditingRecord(null);
          setEditForm({ bloodGroup: "", quantity: "", email: "" });
        } else {
          ...
          <Row>
            <Col xs={12}>
              <div className="d-flex gap-2 align-items-center mb-3" data-aos="fade-left">
                ...
                <span className="text-secondary">{donorInfo.organisationName || "N/A"}</span>
              </div>
              <div className="d-flex gap-2 align-items-center mb-3" data-aos="fade-left">
